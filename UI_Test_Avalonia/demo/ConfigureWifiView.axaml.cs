...
public partial class ConfigureWifiView : UserControl
{
    ...
    private async void ScanButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        {
            StatusTextBlock.Text = "Status: No ESP devices found. Check connection and ensure Serial Monitor is closed.";
    ...
    private List<SerialPort> ScanForAllEsps()
    {
        ...
        foreach (var portName in portNames)
        {
            ...
            try
            {
                sp = new SerialPort(portName, 115200) { WriteTimeout = 1000, ReadTimeout = 1000 };
                // Keep DTR and RTS true - this successfully allowed boot messages before
                sp.DtrEnable = true;
                sp.RtsEnable = true;
                sp.Open();

                Debug.WriteLine($"[C#] Opened {portName}. Waiting for ESP32 to boot and tare scales (takes ~3s)...");
                Thread.Sleep(3000); // Give it time to boot up and tare the HX711 scales

                sp.DiscardInBuffer();
                sp.DiscardOutBuffer();

                Debug.WriteLine($"[C#] Pinging {portName} continuously for up to 6 seconds...");

                var timeout = DateTime.Now.AddSeconds(6.0);
                var responseBuffer = "";
                ...
                {
                    if ((DateTime.Now - lastPing).TotalMilliseconds > 500)
                    {
                        sp.Write("PING\n");
                    ...
                    if (sp.BytesToRead > 0)
                    {
                        responseBuffer += sp.ReadExisting();
                        if (responseBuffer.Contains("START_APLIKACJA"))
                        {
                            Debug.WriteLine($"[C#] Handshake successful on {portName}. Keeping port open.");
                            foundPorts.Add(sp);
                ...
                {
                    Debug.WriteLine($"[C#] Handshake FAILED on {portName}. Did not receive 'START_APLIKACJA'.");
                    Debug.WriteLine($"[C#] Full received buffer from {portName}:\n---\n{responseBuffer}\n---");
                    sp.Close();
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[C#] Error probing {portName}: {ex.Message}. Closing port.");
                sp?.Close();
            }
        }
        return foundPorts;
